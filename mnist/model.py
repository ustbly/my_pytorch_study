...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.layer2 = nn.Sequential(
            nn.Conv2d(16, 32, kernel_size=3),  # (batch,32,24,24)
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2)   # (batch,32,12,12)
        )
        self.layer3 = nn.Sequential(
            nn.Conv2d(32, 64, kernel_size=3),   # (batch,64,10,10)
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True)
        )
        self.layer4 = nn.Sequential(
            nn.Conv2d(64, 128, kernel_size=3),  # (batch,128,8,8)
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
